...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    indices = np.random.choice(num_of_examples, BATCH_SIZE)
    
    data = num_to_word_embedding[in_word_index[indices]]
    labels[range(labels.shape[0]), out_word_index[indices]] = 1
...
def eval_neural_lm(eval_data_path):
    ...
    for i in xrange(num_of_examples):
        total_prob += np.log2(forward(num_to_word_embedding[in_word_index[i]], out_word_index[i], params, dimensions))
    l = total_prob / num_of_examples
