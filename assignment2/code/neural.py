...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    # forward is just computing the cross entropy function.
    z1      = data.dot(W1) + b1
    h       = sigmoid(z1)
    z2      = h.dot(W2) + b2
    y_hat   = softmax(z2)
    ...
    softmax_grad = y_hat - labels
    
    gradb2 = np.sum(softmax_grad, axis=0)
    gradW2 = h.transpose().dot(softmax_grad)
    gradb1 = np.sum(softmax_grad.dot(W2.transpose()) * sigmoid_grad(h), axis=0)
    gradW1 = data.transpose().dot((softmax_grad.dot(W2.transpose()) * sigmoid_grad(h)))   
